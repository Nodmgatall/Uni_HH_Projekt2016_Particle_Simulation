...
void AlgorithmStoermerVerletLennardJones::step_2 (const data_type&       p_position_aix,
    ...
    std::vector<data_type> distances_x(count_of_dist_calculations);
    std::vector<data_type> distances_y(count_of_dist_calculations);
    std::vector<data_type> distances_z(count_of_dist_calculations);
    for(unsigned long i = 0; i < count_of_dist_calculations; i++)
    {
        unsigned long other = p_index_j_begin + i;
        distances_x[i] = p_position_ajx[other] - p_position_aix;
        distances_x[i] *= distances_x[i];
        distances_y[i] = p_position_ajy[other] - p_position_aiy;
        distances_y[i] *= distances_y[i];
        distances_z[i] = p_position_ajz[other] - p_position_aiz;
        distances_z[i] *= distances_z[i];
    }
    unsigned long j;
    for (j = 0; j < count_of_dist_calculations; j++) {
        // Distance vector calculation
        // distance scalar squared calc
        const data_type r_ij_2  = distances_x[j] * distances_x[j] + distances_y[j] * distances_y[j] + distances_z[j] * distances_z[j];
        ...
        const data_type s_ij_x  = s_ij * distances_x[j];
        const data_type s_ij_y  = s_ij * distances_y[j];
        const data_type s_ij_z  = s_ij * distances_z[j];
        p_position_bix -= s_ij_x / m_i;
        p_position_biy -= s_ij_y / m_i;
        p_position_biz -= s_ij_z / m_i;
        p_position_bjx[j+p_index_j_begin] += s_ij_x / m_j;
        p_position_bjy[j+p_index_j_begin] += s_ij_y / m_j;
        p_position_bjz[j+p_index_j_begin] += s_ij_z / m_j;
    }
}
