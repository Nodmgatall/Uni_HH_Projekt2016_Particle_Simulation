...
public:
	...
	GeneratorInvalidException(int p_generator_mode) :
			m_generator_mode(p_generator_mode) {
	}
	virtual const char *what() const throw () {
		char *text = (char*) malloc(100);
		sprintf(text, "generator mode ( %d ) is undefined",
				m_generator_mode);
		return text;
	}
};
...
public:
	GeneratorNotImplementedException(int p_generator_mode) :
			m_generator_mode(p_generator_mode) {
	}
	virtual const char *what() const throw () {
		char *text = (char*) malloc(100);
		sprintf(text, "generator mode ( %d ) is not (fully) implemented",
				m_generator_mode);
		return text;
	}
};
